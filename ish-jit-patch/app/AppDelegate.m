...
// ---------------------------------------------------------------------------
// JIT initialisation — waits for StikDebug to set CS_DEBUGGED, then inits
// ---------------------------------------------------------------------------

#define CS_DEBUGGED 0x10000000u  // Set when a debugger is attached

static void jit_wait_and_init(void) {
    // Poll for CS_DEBUGGED up to 10 seconds (StikDebug attaches within ~2s)
    ...
    for (int i = 0; i < max_attempts; i++) {
        ...
        if (csops(getpid(), 0, &flags, sizeof(flags)) == 0) {
            if (flags & CS_DEBUGGED) {
                NSLog(@"[iSH-JIT] CS_DEBUGGED set (attempt %d). Initialising JIT.", i + 1);
                int rc = jit_init();
                if (rc == 0) {
                    NSLog(@"[iSH-JIT] MAP_JIT succeeded. JIT active on A17/TXM.");
                } else {
                    NSLog(@"[iSH-JIT] MAP_JIT failed. Running Asbestos interpreter.");
                }
                return;
            }
        }
        [NSThread sleepForTimeInterval:poll_ms / 1000.0];
    }
    NSLog(@"[iSH-JIT] Timeout: StikDebug not attached. Running Asbestos interpreter.");
    // JIT stays uninitialised — iSH works correctly without it
}
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    // Launch JIT initialisation in background thread.
    // jit_wait_and_init() polls for StikDebug (CS_DEBUGGED) then calls jit_init().
    // iSH boots and works immediately via Asbestos; JIT kicks in once debugger attaches.
